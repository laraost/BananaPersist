/*
 * This file is from `github.com/weinkauf/Persistence1D`;
 * Original authors are Yeara Kozlov and Tino Weinkauf.
 *
 * There are some alterations compared to the original code:
 *  - using `double` instead of `float`
 *  - a static_cast from int to size_t, because I don't want to change all int to size_t
 */
	...
	*/
	void Watershed()
	{
		...
		for (std::vector<TIdxAndData>::iterator p = SortedData.begin(); p != SortedData.end(); p++)
		{
			...
			else if (static_cast<size_t>(i) == Colors.size()-1) //right most vertex - look only to the left
			{
				if (Colors[i-1] == NO_COLOR) 
				{
					CreateComponent(i);
